...
- 다음 단계로 이동하는 이유와 연결 근거를 제공합니다. 실제 코드 인용, 확인하지 못한 부분이 있는 추론, 이해를 위한 읽기 순서를 구분합니다.
...
- 데스크톱에서는 두 세로 경계를 드래그해 바로 옆 영역들의 폭을 조절합니다. 설명·코드 순서를 바꿔도 영역별 폭을 유지하고, 브라우저 저장소를 사용할 수 있으면 기억합니다. 최소 폭을 보장하며 창이 작아지면 임시로 줄입니다. 경계를 더블클릭하면 전체 폭을 초기화합니다. 키보드로 경계에 초점을 옮긴 뒤 방향키(Shift로 큰 폭), Home/End로 조절하고 Enter로 초기화할 수도 있습니다. 드래그 중 Escape는 조절을 취소합니다.
...
각 단계 끝의 **다음** 영역에서 왜 다음 코드를 읽는지 확인합니다. **코드 근거 첨부**에서는 변경하지 않은 연결 코드까지 펼쳐볼 수 있습니다. **추론 포함**은 확인하지 못한 부분을 설명하고, **이해를 위한 읽기 순서**는 구현에서 테스트로 이동하는 것처럼 읽기 목적의 연결을 나타냅니다.

근거마다 변경 전·후 구분, 줄 번호, 구문 강조와 고정 커밋 원본 링크가 있습니다. 인용 코드는 HTML에 포함되어 오프라인에서도 읽을 수 있습니다. 가이드 전체 연결 요약은 작성된 근거·추론·읽기 순서·미작성 개수를 보여줍니다. 실행 경로의 완전성이나 정확성을 보증하는 숫자는 아닙니다. 빌더는 인용한 원본과 위치를 검사하고, 연결의 해석은 에이전트가 작성합니다.

예제의 [테스트 클라이언트 연결](https://yunsinlee.github.io/pr-tour/demo.ko.html#finish-body)에서는 메시지가 큐를 거치는 코드와 아직 인용하지 않은 프레임워크 연결을 함께 볼 수 있습니다. 기존 가이드에 추가하려면 manifest에 연결 설명을 작성한 뒤 HTML을 다시 생성하세요. 재생성만으로 설명이 자동 작성되지는 않습니다. 자세한 형식은 [연결 정보 작성법](skills/pr-tour/references/manifest.md#connections-between-steps)을 참고하세요.
...
두 데모의 영역 폭 조절·모바일 읽기·정의 미리보기·코멘트·연결 근거 브라우저 회귀 테스트는 Node.js 20 이상에서 실행합니다.
