...
저장한 코멘트는 PR과 기준 커밋별로 **이 브라우저의 저장소**에 보관됩니다. HTML 파일을 수정하지 않으며 다른 기기나 브라우저로 동기화되지 않습니다. 파일 주소로 열거나 앱 안에서 볼 때는 저장소가 제한될 수 있습니다. 화면의 저장 상태를 확인하고, 오래 보관하거나 공유하려면 JSON을 다운로드하세요. 현재 JSON 다시 불러오기는 지원하지 않습니다. 저장 전 작성 중인 글은 같은 페이지를 닫지 않는 동안만 다시 열 수 있습니다.
