...
- 기본 배치는 **설명 → 코드**입니다. 상단 **화면 배치**에서 **코드 → 설명**으로 바꿀 수 있고, 브라우저 저장소를 사용할 수 있으면 선택을 기억합니다. 휴대폰과 좁은 화면에서는 **설명 / 코드** 탭으로 한 화면씩 읽으며, 탭을 바꿔도 읽던 위치를 유지합니다. 설명을 누르면 관련 코드로 이동하고, 다른 구간으로 넘어가면 설명부터 표시합니다.
- 모바일에서는 코드 줄바꿈을 선택할 수 있고, 단계 이동 버튼과 정의 미리보기를 터치에 맞게 넓혔습니다. 휴대폰 가로 화면에서는 헤더를 줄여 코드를 읽을 공간을 확보합니다.
...
두 데모의 모바일 읽기·정의 미리보기 브라우저 회귀 테스트는 Node.js 20 이상에서 실행합니다.
