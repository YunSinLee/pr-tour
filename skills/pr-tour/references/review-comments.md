...
Verify ranges on both sides, a renamed path, hidden context navigation, reload persistence, snapshot isolation, storage denial, clipboard denial, JSON round trips and source fidelity, duplicate/conflict handling, atomic import rejection, multi-tab save conflicts, and mobile dialog/selection controls when changing this feature.
