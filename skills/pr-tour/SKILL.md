...
- Let readers select line ranges and save local review comments, edit/delete them, return to their code, and copy or download JSON for an AI conversation. Preserve the storage-status notice and clipboard fallback. See [reader comments and export](references/review-comments.md) when modifying or consuming this feature; reader comments are separate from authored manifest notes.
